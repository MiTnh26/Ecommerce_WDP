...
const HomePage = () => {
  ...
  const { ref: newProductRef, inView: newProductRefInView } = useInView({
    triggerOnce: true,
  });
  ...
  const handleCategoryClickNext = async () => {
    ...
    try {
      const res = await axios.get(
        `${baseURL}/category/get-category?limit=1&skip=${dataCategory.length}`,
        {
  ...
  return (
    <div className="mt-4">
        ...
        >
          ...
          <div style={{ height: "340px" }} className="mt-auto">
            ...
            >
              ...
              >
                View Now <i className="fa-solid fa-arrow-right"></i>
              </a>
            </div>
          </div>
        </Col>
      ...
      {/* Bannner 2 */}
      <div
        className="d-flex gap-3 mt-5 bg-white rounded-3 shadow-sm py-2 px-1"
        style={{ height: "200px" }}
      >
        <div
          className="banner2 rounded-3"
        ...
        {isBestSellerLoading ? (
          ...
          <>
            {bestSellerData && bestSellerData.length > 0 ? (
              <>
                <Category
                  dataList={bestSellerData}
                  title="Best Seller Products"
                  Component={Card}
                  dataLength={bestSellerData.length}
                />
              </>
            ) : (
              <>
      ...
      {/* New product */}
      <div
        className="new-product mt-5 bg-white rounded-3 shadow-sm py-2 px-1"
        ref={newProductRef}
      >
        {isNewProductLoading ? (
          <>
          ...
          <>
            {newProductData && newProductData.length > 0 ? (
              <>
                <Category
                  dataList={newProductData}
                  title="New Products"
                  Component={Card}
                  dataLength={newProductData.length}
                />
              </>
            ) : (
              <>
